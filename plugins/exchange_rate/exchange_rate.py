...
class BitcoinVenezuela(ExchangeBase):
    def get_rates(self, ccy):
        ...
        rates = [(r, json['ZEIT'][r]) for r in json['ZEIT']
                 if json['ZEIT'][r] is not None]  # Giving NULL for LTC
        return dict(rates)
    ...
    def historical_rates(self, ccy):
        return self.get_json('api.bitcoinvenezuela.com',
                             "/historical/index.php?coin=ZEIT")[ccy +'_ZEIT']
...
class FxPlugin(BasePlugin, ThreadJob):
    ...
    def get_fiat_status_text(self, btc_balance):
        rate = self.exchange_rate()
        return _("  (No FX rate available)") if rate is None else "1 ZEIT~%s %s" % (self.value_str(COIN, rate), self.ccy)
